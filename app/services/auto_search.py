...
async def _limit_torrent_files_to_episodes(
    ...
    if wanted_indices:
        ...
        # Гарантированно заносим раздачу в черный список и список исключений
        t_name = getattr(torrent, "name", "")
        if effective_show_id:
            if torrent_hash:
                add_rejected_release_for_show(effective_show_id, torrent_hash)
            if t_name:
        ...
        try:
            ...
            if effective_show_id:
                ...
                retry_ids = explicit_episode_ids or target_db_ids or {ep.id for ep in target_eps if getattr(ep, "id", None)}
                asyncio.create_task(_retry_next_candidate(effective_show_id, retry_ids))
        except Exception as exc:
            logger.debug("Не удалось занести в черный список или запланировать автопоиск следующего кандидата: %s", exc)
    ...
    def covers(c, ep: Episode) -> bool:
        ...
        if show.content_type == "movie":
            ...
            if s_lbl["type"] == "numbered":
                ...
                alias_has_num = False
                if alias_cand and s_num is not None:
                    s_num_str = str(s_num)
                    alias_has_num = (
                        re.search(r"\b" + re.escape(s_num_str) + r"\b", alias_cand.text) is not None
                        or (s_num == 2 and re.search(r"\b(?:II|2)\b", alias_cand.text, re.IGNORECASE) is not None)
                        or (s_num == 3 and re.search(r"\b(?:III|3)\b", alias_cand.text, re.IGNORECASE) is not None)
                        or (s_num == 4 and re.search(r"\b(?:IV|4)\b", alias_cand.text, re.IGNORECASE) is not None)
