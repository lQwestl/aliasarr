...
# Диапазоны в скобках или явные: [01-06], [1-6], [154-175], [E01-E12], [01-08 из 12], [01-08 из XX], [E01-E12 of 12], (1-12 из 24)
_RE_BRACKET_RANGE = re.compile(
    r"\[\s*(?:(?:ep|эп|сери[ия]|episode|e)\.?\s*)?(\d{1,4})\s*[-–~]\s*(?:(?:ep|эп|сери[ия]|episode|e)\.?\s*)?(\d{1,4})(?:\s*(?:из|of|iz|\/|\|)\s*(?:(?:ep|эп|сери[ия]|episode|e)\.?\s*)?(?:\d{1,4}|[xX?]+)\+?)?(?:\s*(?:эп(?:изод(?:ов|а)?)?|сери[йия]|eps?|episodes?|выпуск(?:ов|а)?))?\s*\]",
    re.IGNORECASE,
)
_RE_RANGE_IZ_N = re.compile(
    r"(?:\[|\(|\b)(?:(?:ep|эп|сери[ия]|episode|e)\.?\s*)?(\d{1,4})\s*(?:[-–~]|to)\s*(?:(?:ep|эп|сери[ия]|episode|e)\.?\s*)?(\d{1,4})\s*(?:из|of|iz|\/|\|)\s*(?:(?:ep|эп|сери[ия]|episode|e)\.?\s*)?(?:\d{1,4}|[xX?]+)\+?(?:\s*(?:эп(?:изод(?:ов|а)?)?|сери[йия]|eps?|episodes?|выпуск(?:ов|а)?))?(?:\]|\)|\b)",
    re.IGNORECASE,
)
_RE_SINGLE_IZ_N = re.compile(
    r"(?:\[|\()(?:(?:ep|эп|сери[яи]|episode|e)\.?\s*)?(\d{1,4})\s*(?:из|of|iz|\/|\|)\s*(?:(?:ep|эп|сери[яи]|episode|e)\.?\s*)?(?:\d{1,4}|[xX?]+)\+?(?:\s*(?:эп(?:изод(?:ов|а)?)?|сери[йия]|eps?|episodes?|выпуск(?:ов|а)?))?(?:\]|\))|"
    r"\b(?:(?:ep|эп|сери[яи]|episode|e)\.?\s*)(\d{1,4})\s*(?:из|of|iz|\/|\|)\s*(?:(?:ep|эп|сери[яи]|episode|e)\.?\s*)?(?:\d{1,4}|[xX?]+)\+?(?:\s*(?:эп(?:изод(?:ов|а)?)?|сери[йия]|eps?|episodes?|выпуск(?:ов|а)?))?\b|"
    r"\b(\d{1,4})\s*(?:из|of|iz)\s*(?:(?:ep|эп|сери[яи]|episode|e)\.?\s*)?(?:\d{1,4}|[xX?]+)\+?(?:\s*(?:эп(?:изод(?:ов|а)?)?|сери[йия]|eps?|episodes?|выпуск(?:ов|а)?))?\b",
    re.IGNORECASE,
)
