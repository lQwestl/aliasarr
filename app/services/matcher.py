...
NON_VIDEO_KEYWORDS = re.compile(
    ...
    r"\b(?:wavpack|ape|alac|dxd|sacd|dsd\d*|vinyl|audio\s*cd|maxi[-_\s]?single|mini[-_\s]?album)\b|\[ep\]|\(ep\)|\[single\]|\(single\)|"
    ...
    # Музыкальные жанры распознаются как не-видео только внутри скобок/тегов либо в сочетании с аудио-маркерами
    r"[\[\(][^\]\)]*\b(?:death\s*metal|black\s*metal|heavy\s*metal|thrash\s*metal|power\s*metal|doom\s*metal|gothic\s*metal|folk\s*metal|speed\s*metal|hard\s*rock|prog\s*rock|krautrock|psychedelic|occult\s*rock|euro[-_\s]?disco|synth[-_\s]?pop|electro\s*house|synthwave|italodance|ambient|classical|techno|trance|hip[-_\s]?hop)\b[^\]\)]*[\]\)]|"
    r"\b(?:death\s*metal|black\s*metal|heavy\s*metal|hard\s*rock|prog\s*rock|krautrock|psychedelic|occult\s*rock|euro[-_\s]?disco|synth[-_\s]?pop|ambient|trance|hip[-_\s]?hop|synthwave|techno)\b(?=.*(?:\b(?:flac|mp3|lossless|alac|wavpack|ape|320\s*kbps|tracks\+?\.?cue|image\+?\.?cue|discography|дискография)\b|\[(?:32/\d+|24/\d+|12\"|dxd|tr\d+|vinyl|lp|cd|flac|mp3|lossless|audio)\]|\((?:12\"|32/\d+|24/\d+|tracks|image\+\.cue|tracks\+\.cue|wavpack|flac)\)))|"
    ...
    r"\b(?:repack|русификатор|gamesvoice|ps\s*vr|steam|gog|duplex|codex|tenoke|razor1911|dlc|eshop|portable|insaneramzes|pSyPSP|arcade\s*games?|игров(?:ой\s+автомат|ые\s+автоматы))\b|"
    r"\[(?:игровой\s+автомат|arcade\s*game|strike|suxxors|unlimited)\]|"
...
def match_release(
    ...
    if content_type == "movie":
        # Проверяем, является ли релиз спешл-паком, OVA, антологией или сборником фильмов
        is_special_or_pack = bool(
            re.search(
                r"\[(?:Special|Specials|OVA|ONA|Movie\+Sp|Movie\s*\+\s*Special|Movie\s*Pack|Filmpack|Sp)\]|\b(?:Special|Specials|OVA|ONA|Collection|Антология|Трилогия|Дилогия|Дулогия|Коллекция|Movie\s*Collection|Movie\s*Pack|Filmpack|Фильмы\s*\d+[-–]\d+)\b",
                release_name,
                re.IGNORECASE,
            )
            or ("/" in release_name and score >= 80)
        )

        if not is_special_or_pack:
            ...
            # Для спешлов и сборников фильмов отсекаем только явные многосезонные регулярные ТВ-сериалы (Season 2-4)
            s_lbl = detect_season_label(release_name)
            if s_lbl["type"] == "range":
                return MatchResult(
                    matched=False, show_id=None, alias_id=None, alias_text=None,
            ...
            if parsed.kind == ReleaseKind.SEASON_PACK and parsed.season is not None and parsed.season > 0:
                return MatchResult(
                    matched=False, show_id=None, alias_id=None, alias_text=None,
                    score=score, parsed=parsed,
                )
